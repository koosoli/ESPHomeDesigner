...
type DeviceProfilesMap = Record<
  string,
  Partial<DeviceProfile> & Record<string, any>
>;
...
export class AppStateFacade {
  ...
  get settings(): AppSettingsSnapshot {
    return {
      ...
      protocolHardware: this.project.protocolHardware as Record<
        string,
        any
      > | null,
      protocol_hardware: this.project.protocolHardware as Record<
        string,
        any
      > | null,
      ...this.secrets.keys,
  ...
  getSelectedWidget(): Widget | undefined {
    return this.project.getWidgetById(this.editor.selectedWidgetIds[0]) as
      | Widget
      | undefined;
  }
  ...
  getSelectedProfile(): DeviceProfile | null {
    ...
    return this.project.deviceModel
      ? (profiles[this.project.deviceModel] as DeviceProfile | undefined) ||
          null
      : null;
  }
  ...
  getCanvasDimensions(): { width: number; height: number } {
    ...
    if (mode === "oepl" || mode === "opendisplay") {
      const protocolHardware = this.project.protocolHardware as {
        width: number;
        height: number;
      };
    ...
    return this.project.getCanvasDimensions(
      this.preferences.state.orientation || "landscape",
    );
  }
  ...
  getPagesPayload(): ProjectPayload {
    ...
    payload.customHardware = this.project.customHardware as
      | HardwareSettings
      | undefined;
    payload.protocolHardware = this.project.protocolHardware as
      | Record<string, any>
      | undefined;

    payload.device_model = this.project.deviceModel || undefined;
    payload.custom_hardware = this.project.customHardware as
      | HardwareSettings
      | undefined;
    payload.protocol_hardware = this.project.protocolHardware as
      | Record<string, any>
      | undefined;
    return payload;
  }
  ...
  updateSettings(newSettings: AppSettingsSnapshot): void {
    ...
    if (Object.keys(secretUpdates).length) {
      Object.entries(secretUpdates).forEach(([key, value]) =>
    ...
    if (
      newSettings.device_model ||
      newSettings.orientation ||
      newSettings.custom_hardware
    ) {
      emit(EVENTS.PAGE_CHANGED, {
        index: this.currentPageIndex,
        forceFocus: true,
  ...
  setManualYamlOverride(
    value: string,
    options: ManualYamlOverrideOptions = {},
  ): void {
  ...
  updateLayoutIndicator(): void {
    ...
    if (nameElement) {
      nameElement.textContent =
        this.project.deviceName || this.project.currentLayoutId || "Unknown";
    }
  }
...
const handler: ProxyHandler<AppStateFacade> = {
  set(target, prop, value, receiver) {
    if (prop === "snapEnabled") {
      Logger.warn(
        `[StateProxy] Intercepted illegal write to '${String(prop)}'. Automatically rerouting to setSnapEnabled().`,
      );
    ...
    if (
      typeof prop === "string" &&
      !allowedInternalProps.includes(prop) &&
      typeof target[prop as keyof AppStateFacade] !== "function"
    ) {
      Logger.warn(
        `[StateProxy] Illegal state mutation detected: AppState.${prop} = ${String(value)}`,
      );
      console.trace(
        `[StateProxy] Trace for illegal mutation of AppState.${prop}`,
      );
    }
