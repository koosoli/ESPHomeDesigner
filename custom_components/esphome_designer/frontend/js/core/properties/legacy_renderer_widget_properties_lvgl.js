...
import { renderFontControls, FONT_OPTIONS } from './legacy_renderer_ui_helpers.js';

const LVGL_ALIGN_DEFAULT = 'CENTER';
...
function renderLvglLabelSection(panel, props, updateProp) {
    ...
    renderFontControls(panel, props, updateProp, { alignDefault: LVGL_ALIGN_DEFAULT });
    if (needsCustomFontHint(props.font_family)) {
        panel.addHint('Browse fonts.google.com');
    }
}

function renderLvglLineSection(panel, widget, props, updateProp) {
    const resolution = AppState.getCanvasDimensions();
    const canvasW = resolution.width;
    const canvasH = resolution.height;

    panel.addSelect('Orientation', props.orientation || 'horizontal', ['horizontal', 'vertical'], (v) => {
...
function renderLvglMeterSection(panel, widget, props, updateProp) {
    ...
    panel.addHint('Meter widgets must be square. Width and height are locked together.');
    panel.endSection();
...
function renderLvglButtonSection(panel, widget, props, updateProp) {
    ...
    panel.addHint('Entity to toggle or trigger when clicked');
...
function renderLvglArcSection(panel, widget, props, updateProp) {
...
function renderLvglChartSection(panel, widget, props, updateProp) {
    panel.addLabeledInputWithPicker('Entity ID', 'text', widget.entity_id || '', (v) => AppState.updateWidget(widget.id, { entity_id: v }), widget);
...
function renderLvglImageSection(panel, props, updateProp) {
...
function renderLvglQrcodeSection(panel, props, updateProp) {
...
function renderLvglBarSection(panel, widget, props, updateProp) {
    panel.addLabeledInputWithPicker('Entity ID', 'text', widget.entity_id || '', (v) => AppState.updateWidget(widget.id, { entity_id: v }), widget);
...
function renderLvglSliderSection(panel, widget, props, updateProp) {
    panel.addLabeledInputWithPicker('Entity ID', 'text', widget.entity_id || '', (v) => AppState.updateWidget(widget.id, { entity_id: v }), widget);
    panel.addSegmentedControl(
        'Orientation',
        [{ value: 'Horizontal', label: 'Horiz', icon: 'mdi-arrow-left-right' }, { value: 'Vertical', label: 'Vert', icon: 'mdi-arrow-up-down' }],
...
function renderLvglTabviewSection(panel, props, updateProp) {
...
function renderLvglCheckboxSection(panel, widget, props, updateProp) {
    panel.addLabeledInputWithPicker('Entity ID', 'text', widget.entity_id || '', (v) => AppState.updateWidget(widget.id, { entity_id: v }), widget);
...
function renderLvglDropdownSection(panel, props, updateProp) {
...
function renderLvglSwitchSection(panel, widget, props, updateProp) {
    panel.addLabeledInputWithPicker('Entity ID', 'text', widget.entity_id || '', (v) => AppState.updateWidget(widget.id, { entity_id: v }), widget);
    panel.addCheckbox('Checked', props.checked || false, (v) => updateProp('checked', v));
...
function renderLvglTextareaSection(panel, props, updateProp) {
...
export function renderLvglLegacyProperties(panel, widget, type, props, updateProp) {
    ...
    if (type === 'lvgl_label') {
        renderLvglLabelSection(panel, props, updateProp);
    } else if (type === 'lvgl_line') {
        renderLvglLineSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_meter') {
        renderLvglMeterSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_button') {
        renderLvglButtonSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_arc') {
        renderLvglArcSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_chart') {
        renderLvglChartSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_img') {
        renderLvglImageSection(panel, props, updateProp);
    } else if (type === 'lvgl_qrcode') {
        renderLvglQrcodeSection(panel, props, updateProp);
    } else if (type === 'lvgl_bar') {
        renderLvglBarSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_slider') {
        renderLvglSliderSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_tabview') {
        renderLvglTabviewSection(panel, props, updateProp);
    } else if (type === 'lvgl_checkbox') {
        renderLvglCheckboxSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_dropdown') {
        renderLvglDropdownSection(panel, props, updateProp);
    } else if (type === 'lvgl_switch') {
        renderLvglSwitchSection(panel, widget, props, updateProp);
    } else if (type === 'lvgl_textarea') {
        renderLvglTextareaSection(panel, props, updateProp);
    }
