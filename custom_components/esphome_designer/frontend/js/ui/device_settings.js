...
import {
  populateDeviceSelectView,
  updateDeviceSettingsVisibility,
} from "./device_settings_view.js";
import {
...
export class DeviceSettings {
  constructor() {
    ...
    /** @param {string} id */
    const getSelect = (id) =>
      /** @type {HTMLSelectElement | null} */ (document.getElementById(id));
    ...
    this.deepSleepStayAwakeEntityInput = getInput(
      "setting-deep-sleep-stay-awake-entity",
    );

    // Network
    this.httpBufferSizeInput = getInput("setting-http-buffer-size");
    ...
    this.deepSleepStayAwakeEntityRow = getElement(
      "deep-sleep-stay-awake-entity-row",
    );
  ...
  init() {
    ...
    const reloadBtn = /** @type {HTMLElement | null} */ (
      document.getElementById("reloadHardwareBtn")
    );
    if (reloadBtn) {
    ...
    const importBtn = /** @type {HTMLElement | null} */ (
      document.getElementById("importHardwareBtn")
    );
    const fileInput = /** @type {HTMLInputElement | null} */ (
      document.getElementById("hardwareFileInput")
    );
  ...
  _setupBasicSettingsListeners() {
    ...
    if (orientationInput) {
      orientationInput.addEventListener("change", () =>
        updateSetting("orientation", orientationInput.value),
      );
    }
    const darkModeInput = this.darkModeInput;
    if (darkModeInput) {
      darkModeInput.addEventListener("change", () =>
        updateSetting("darkMode", darkModeInput.checked),
      );
    }
    const invertedColorsInput = this.invertedColorsInput;
    if (invertedColorsInput) {
      invertedColorsInput.addEventListener("change", () =>
        updateSetting("invertedColors", invertedColorsInput.checked),
      );
    }
  ...
  _setupPowerSettingsListeners() {
    ...
    if (sleepStart)
      sleepStart.addEventListener("change", () =>
        updateSetting("sleepStartHour", parseInt(sleepStart.value) || 0),
      );
    if (sleepEnd)
      sleepEnd.addEventListener("change", () =>
        updateSetting("sleepEndHour", parseInt(sleepEnd.value) || 0),
      );
    if (dailyRefreshTime)
      dailyRefreshTime.addEventListener("change", () =>
        updateSetting("dailyRefreshTime", dailyRefreshTime.value),
      );
    ...
    if (noRefreshStart)
      noRefreshStart.addEventListener("change", () =>
        updateSetting(
          "noRefreshStartHour",
          noRefreshStart.value === "" ? null : parseInt(noRefreshStart.value),
        ),
      );
    if (noRefreshEnd)
      noRefreshEnd.addEventListener("change", () =>
        updateSetting(
          "noRefreshEndHour",
          noRefreshEnd.value === "" ? null : parseInt(noRefreshEnd.value),
        ),
      );
    ...
    if (deepSleepStayAwakeEntityInput) {
      deepSleepStayAwakeEntityInput.addEventListener("change", () => {
        const value =
          deepSleepStayAwakeEntityInput.value.trim() ||
    ...
    if (fwGuardCb)
      fwGuardCb.addEventListener("change", () =>
        updateSetting("deepSleepFirmwareGuard", fwGuardCb.checked),
      );
  }
  ...
  _setupCycleAndEcoListeners() {
    ...
    if (autoCycleInterval) {
      autoCycleInterval.addEventListener("input", () =>
        updateSetting(
          "autoCycleIntervalS",
          Math.max(5, parseInt(autoCycleInterval.value) || 30),
        ),
      );
    }

    const lcdStrategyRadios = /** @type {NodeListOf<HTMLInputElement>} */ (
      document.querySelectorAll('input[name="lcdEcoStrategy"]')
    );
    lcdStrategyRadios.forEach((radio) => {
      radio.addEventListener("change", () => {
        if (radio.checked) {
          ...
          if (this.sleepRow)
            this.sleepRow.style.display =
              radio.value === "backlight_off" ? "flex" : "none";
          if (this.dimTimeoutRow)
            this.dimTimeoutRow.style.display =
              radio.value === "dim_after_timeout" ? "flex" : "none";
        }
      });
    });
    ...
    if (dimTimeoutInput)
      dimTimeoutInput.addEventListener("input", () =>
        updateSetting("dimTimeout", parseInt(dimTimeoutInput.value) || 10),
      );

    const httpBufferSizeInput = this.httpBufferSizeInput;
    if (httpBufferSizeInput)
      httpBufferSizeInput.addEventListener("input", () =>
        updateSetting(
          "httpBufferSizeRx",
          parseInt(httpBufferSizeInput.value) || 4096,
        ),
      );
  }
