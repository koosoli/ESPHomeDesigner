...
export function generateDisplayLambda(pages, layout, profile, context, adapter) {
    ...
    lines.push("// Helper to split text into wrapped lines at widget boundary");
    lines.push("auto wrap_text_lines = [&](int max_w, esphome::font::Font *font, const char* text, std::vector<std::string> &lines) {");
    lines.push("  lines.clear();");
    lines.push("  if (!text || max_w <= 0) return;");
    lines.push("  std::string line;");
    lines.push("  std::string word;");
    lines.push("  const char* p = text;");
    lines.push("  auto commit_word = [&]() {");
    lines.push("    if (word.empty()) return;");
    lines.push("    std::string cand = line.empty() ? word : (line + \" \" + word);");
    lines.push("    int cw = 0, cx_b = 0, cbl = 0, ch = 0;");
    lines.push("    font->measure(cand.c_str(), &cw, &cx_b, &cbl, &ch);");
    lines.push("    if (cw > max_w && !line.empty()) {");
    lines.push("      lines.push_back(line);");
    lines.push("      line = word;");
    lines.push("    } else {");
    lines.push("      line = cand;");
    lines.push("    }");
    ...
    lines.push("    }");
    lines.push("    if (*p == '\\n') {");
    lines.push("      commit_word();");
    lines.push("      lines.push_back(line);");
    lines.push("      line.clear();");
    lines.push("    } else if (*p == ' ' || *p == '\\t') {");
    lines.push("      commit_word();");
    lines.push("    } else {");
    lines.push("      word += *p;");
    lines.push("    }");
    ...
    lines.push("  std::vector<std::string> lines;");
    lines.push("  wrap_text_lines(max_w, font, text, lines);");
    lines.push("  if (lines.empty()) return;");
    lines.push("  int cy = y;");
    lines.push("  if (align == TextAlign::CENTER || align == TextAlign::CENTER_LEFT || align == TextAlign::CENTER_RIGHT) {");
    lines.push("    cy = y - ((int)lines.size() - 1) * line_h / 2;");
    lines.push("  } else if (align == TextAlign::BOTTOM_LEFT || align == TextAlign::BOTTOM_CENTER || align == TextAlign::BOTTOM_RIGHT) {");
    lines.push("    cy = y - ((int)lines.size() - 1) * line_h;");
    lines.push("  }");
    lines.push("  for (const auto &l : lines) {");
    lines.push("    it.print(x, cy, font, color, align, l.c_str());");
