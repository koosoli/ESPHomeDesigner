...
export class YamlGenerator {
    ...
    generateInstructionHeader(profile, layout) {
        ...
        if (feats.psram && (profile.chip?.includes("s3") || profile.id?.includes("s3"))) {
            frameworkHint = "ESP-IDF with execute_from_psram";
        }
        ...
        lines.push("#         - Framework: ESP-IDF (Required for stable PSRAM/LVGL)");
        lines.push("#");
        lines.push("# STEP 3: REPLACE your entire YAML with this generated output");
        lines.push("#         (This profile includes full esphome: and esp32: sections)");
        lines.push("#");
        lines.push("# ============================================================================");
        lines.push("");
        lines.push("# ============================================================================");
        ...
        lines.push("#   # Fix Issue #80: Call display refresh script on boot to prevent black screen");
        lines.push("#   on_boot:");
        lines.push("#     priority: 600");
        lines.push("#     then:");
        // Fix for Issue #182: Validate restored display_page to prevent touch deadlock
        if (layout.pages && layout.pages.length > 0) {
            lines.push("#       - lambda: |-");
            lines.push(`#           if (id(display_page) >= ${layout.pages.length}) {`);
            lines.push(`#             ESP_LOGW("display", "Restored page index %d is out of bounds (max ${layout.pages.length - 1}). Resetting to 0.", id(display_page));`);
