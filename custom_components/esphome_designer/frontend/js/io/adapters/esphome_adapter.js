...
export class ESPHomeAdapter extends BaseAdapter {
    ...
    generateDisplayLambda(pages, layout, profile, context) {
        ...
        lines.push("    // This prevents 'glyph not found' errors for non-renderable characters");
        lines.push("    bool is_space = (*p == ' ' || *p == '\\n' || *p == '\\r' || *p == '\\t');");
        lines.push("    if (is_space) {");
        lines.push("      if (!word.empty()) {");
        lines.push("        int ww, wh, wbl;");
        lines.push("        font->measure(word.c_str(), &ww, &wbl, &wh);");
        lines.push("        int lw = 0;");
        lines.push("        if (!line.empty()) { font->measure(line.c_str(), &lw, &wbl, &wh); lw += font->measure(\" \", &ww, &wbl, &wh); }");
        ...
        lines.push("      }");
        lines.push("      // Explicit newline handling if needed, but for now we treat as flow-break (space)");
        lines.push("      // If specific \\n behavior is desired, logic can be added here.");
        lines.push("    } else {");
        lines.push("      word += *p;");
        lines.push("    }");
        lines.push("    p++;");
