...
`.trim();try{let d="";o==="gemini"?d=await this.callGemini(r,s,a,c):o==="openai"?d=await this.callOpenAI(r,s,a,c):o==="openrouter"&&(d=await this.callOpenRouter(r,s,a,c));let u=d.trim();if(u.includes("```")){const v=u.match(/```(?:json)?\s*([\s\S]*?)\s*```/);v&&v[1]&&(u=v[1].trim())}const g=u.indexOf("["),h=u.indexOf("{");let m=-1,f=-1;g!==-1&&(h===-1||g<h)?(m=g,f=u.lastIndexOf("]")):h!==-1&&(m=h,f=u.lastIndexOf("}")),m!==-1&&f!==-1&&f>m&&(u=u.substring(m,f+1));try{const v=JSON.parse(u),y=qt.validateResponse(v);if(!y.valid&&(_.warn("[AI] Validation failed:",y.errors),y.sanitized.length===0))throw new Error("AI returned invalid widget data: "+y.errors.join(", "));return y.sanitized}catch(v){_.warn("Fast JSON parse failed, trying repair...",v);try{const y=this.repairJson(u),b=JSON.parse(y),x=qt.validateResponse(b);return x.valid||_.warn("[AI] Validation failed after repair:",x.errors),x.sanitized}catch(y){throw _.error("JSON repair failed:",y),new Error("AI returned malformed JSON (possibly truncated). Try a shorter prompt or a more powerful model.")}}}catch(d){throw _.error("AI processing failed:",d),d}}async callGemini(e,n,i,o){const r=`https://generativelanguage.googleapis.com/v1beta/models/${n}:generateContent?key=${e}`,s={contents:[{role:"user",parts:[{text:i+`
...
`).trim()}function ls(t){const e=t.match(/^(?:# ?)?([A-Za-z0-9_]+):(?:\s+#.*)?\s*$/);return e?e[1]:null}function ds(t){const e=String(t||"").replace(/\r\n/g,`
`).split(`
`),n=[];let i=!1;for(const r of e){const s=ls(r);s&&(i=ss.has(s),i)||i||n.push(r)}const o=n.join(`
`).replace(/\n{3,}/g,`

`).trim();if(!o)throw new Error("No UI YAML remains after removing hardware/system sections");return o}function cs(t,e={}){const n=e.oeplEntityId||"open_epaper_link.0000000000000000",i=e.oeplDither??2;t.target.entity_id=n,t.data.dither=i;let o=`service: ${t.service}
`;return o+=`target:
...
`,ys=`    <aside class="sidebar" role="complementary" aria-label="Editor Sidebar">
...
`,bs=`    <aside class="right-panel" role="complementary" aria-label="Widget Properties">
      <!-- Hierarchy Panel -->
    ...
    `,l){const h=o.querySelector(".hierarchy-group-toggle");h&&h.addEventListener("click",m=>{p.updateWidget(e.id,{expanded:e.expanded===!1}),m.stopPropagation()})}const c=o.querySelector(".hierarchy-item-label");c&&c.addEventListener("click",h=>{if((p.selectedWidgetIds||[]).includes(e.id)){const m=prompt("Rename:",a);m!==null&&m!==""&&m!==a&&p.updateWidget(e.id,{title:m}),h.stopPropagation()}}),o.addEventListener("click",h=>{const m=h.ctrlKey||h.shiftKey;p.selectWidget(e.id,m),h.stopPropagation()});const d=o.querySelector(".toggle-lock");d&&d.addEventListener("click",h=>{p.updateWidget(e.id,{locked:!e.locked}),h.stopPropagation()});const u=o.querySelector(".toggle-visibility");u&&u.addEventListener("click",h=>{p.updateWidget(e.id,{hidden:!e.hidden}),h.stopPropagation()});const g=o.querySelector(".delete-widget");return g&&g.addEventListener("click",h=>{confirm(`Delete widget "${a}"?`)&&p.deleteWidget(e.id),h.stopPropagation()}),o.addEventListener("dragstart",h=>{h.dataTransfer&&(t.draggedIndex=n,o.classList.add("dragging"),h.dataTransfer.setData("application/widget-id",e.id),h.dataTransfer.effectAllowed="move")}),o.addEventListener("dragend",()=>{if(o.classList.remove("dragging"),t.draggedIndex=null,!t.listContainer)return;t.listContainer.querySelectorAll(".hierarchy-item").forEach(m=>m.classList.remove("drag-over"))}),o.addEventListener("dragover",h=>{h.dataTransfer&&(h.preventDefault(),h.dataTransfer.dropEffect="move",o.classList.add("drag-over"))}),o.addEventListener("dragleave",()=>{o.classList.remove("drag-over")}),o.addEventListener("drop",h=>{if(!h.dataTransfer)return;h.preventDefault();const m=h.dataTransfer.getData("application/widget-id"),f=o.dataset.id;if(m===f)return;const v=p.getWidgetById(m),y=p.getWidgetById(f);if(!v||!y)return;const b=v.type==="group"?null:y.type==="group"?f:y.parentId||null;if(v.parentId!==b){const S={parentId:b};b&&y.type==="group"?p.updateWidget(m,{...S,expanded:!0}):p.updateWidget(m,S)}const x=parseInt(o.dataset.index||"-1",10);t.draggedIndex!==null&&p.reorderWidget(p.currentPageIndex,t.draggedIndex,x)}),o}function Ys(t){const e=t.filter(i=>i.type==="group"||!i.parentId).reverse(),n=new Map;return t.forEach(i=>{!i.parentId||i.type==="group"||(n.has(i.parentId)||n.set(i.parentId,[]),n.get(i.parentId).push(i))}),{topLevel:e,childrenMap:n}}function Us(t,e,n){const i=t.findIndex(o=>o.id===e);if(i===-1)return!1;if(n==="front"){if(i>=t.length-1)return!1;const[o]=t.splice(i,1);return t.push(o),!0}if(n==="back"){if(i===0)return!1;const[o]=t.splice(i,1);return t.unshift(o),!0}return n==="up"?i>=t.length-1?!1:([t[i],t[i+1]]=[t[i+1],t[i]],!0):n==="down"?i===0?!1:([t[i],t[i-1]]=[t[i-1],t[i]],!0):!1}class Da{constructor(){this.listContainer=null,this.header=null,this.panel=null,this.controlsContainer=null,this.draggedIndex=null,this.render=this.render.bind(this),this.highlightSelected=this.highlightSelected.bind(this)}init(){if(this.listContainer=document.getElementById("hierarchyList"),this.header=document.getElementById("hierarchyHeader"),this.panel=document.getElementById("hierarchyPanel"),!this.listContainer||!this.header||!this.panel){_.error("[HierarchyView] Required DOM elements not found");return}this.controlsContainer=document.createElement("div"),this.controlsContainer.id="hierarchyControls",this.controlsContainer.className="hierarchy-controls",this.controlsContainer.style.padding="8px 8px",this.controlsContainer.style.borderTop="1px solid var(--border-subtle)",this.panel.appendChild(this.controlsContainer),this.bindEvents(),this.render(),this.renderHeaderActions(),_.log("[HierarchyView] Initialized")}renderHeaderActions(){if(!this.header)return;let e=this.header.querySelector(".hierarchy-header-toggles");if(!e){e=document.createElement("div"),e.className="hierarchy-header-toggles";const n=this.header.querySelector(".chevron");this.header.insertBefore(e,n||null);const i=this.createHeaderToggle("mdi-lock-outline","Toggle All Locks",()=>{const r=p.getCurrentPage()?.widgets||[],s=r.every(a=>a.locked);r.forEach(a=>p.updateWidget(a.id,{locked:!s}))}),o=this.createHeaderToggle("mdi-eye-outline","Toggle All Visibility",()=>{const r=p.getCurrentPage()?.widgets||[],s=r.every(a=>a.hidden);r.forEach(a=>p.updateWidget(a.id,{hidden:!s}))});e.appendChild(i),e.appendChild(o)}}createHeaderToggle(e,n,i){const o=document.createElement("div");return o.className="h-toggle",o.title=n,o.innerHTML=`<i class="mdi ${e}"></i>`,o.onclick=r=>{r.stopPropagation(),i()},o}bindEvents(){this.header.addEventListener("click",()=>this.toggleCollapse()),W(C.STATE_CHANGED,this.render),W(C.PAGE_CHANGED,this.render),W(C.SELECTION_CHANGED,this.highlightSelected)}toggleCollapse(){if(!this.panel||!this.header)return;const e=this.panel.classList.toggle("hidden"),n=this.header.querySelector(".chevron");n&&(n.style.transform=e?"rotate(-90deg)":"rotate(0deg)")}highlightSelected(){if(!this.listContainer)return;const e=p.selectedWidgetIds||[];this.listContainer.querySelectorAll(".hierarchy-item").forEach(i=>{e.includes(i.dataset.id)?i.classList.add("selected"):i.classList.remove("selected")}),this.renderControls()}render(){if(!this.listContainer||!this.controlsContainer)return;const e=p.getCurrentPage();if(!e)return;if(this.listContainer.innerHTML="",!e.widgets||e.widgets.length===0){this.listContainer.innerHTML='<div style="font-size: 10px; color: var(--muted); text-align: center; padding: 12px;">No widgets on this page</div>',this.controlsContainer.style.display="none";return}const{topLevel:n,childrenMap:i}=Ys(e.widgets),o=(r,s=0)=>{const a=e.widgets.indexOf(r),l=this.createItem(r,a,s);this.listContainer.appendChild(l);const c=i.get(r.id);c&&r.expanded!==!1&&[...c].reverse().forEach(d=>o(d,s+1))};n.forEach(r=>o(r)),this.highlightSelected(),this.renderControls()}createItem(e,n,i=0){return $s(this,e,n,i)}renderControls(){Ns(this,this.controlsContainer)}moveLayerOrder(e,n){const i=p.getCurrentPage();i&&Us(i.widgets,e.id,n)&&p.setPages(p.pages)}moveToFront(e){this.moveLayerOrder(e,"front")}moveToBack(e){this.moveLayerOrder(e,"back")}moveUp(e){this.moveLayerOrder(e,"up")}moveDown(e){this.moveLayerOrder(e,"down")}}function Ha(t){if(!t.modelInput||!H)return;const e=t.modelInput.value;_.log("[DeviceSettings] Populating dropdown with",Object.keys(H).length,"profiles"),t.modelInput.innerHTML="";const n=vt||[],i=[],o=[];Object.entries(H).forEach(([a,l])=>{l.isCustomProfile||l.isOfflineImport?o.push([a,l]):i.push([a,l])});const r=(a,l)=>{const c=document.createElement("option");c.value=a;let d=l.name||a;d=d.replace(/\s*\(Local\)\s*/gi,"").replace(/\s*\(untested\)\s*/gi,"").trim();const u=[];return(l.isCustomProfile||l.isOfflineImport)&&u.push("Imported"),n.includes(a)||u.push("untested"),u.length>0&&(d+=` (${u.join(", ")})`),c.textContent=d,c};if(i.forEach(([a,l])=>t.modelInput.appendChild(r(a,l))),o.length>0&&i.length>0){const a=document.createElement("option");a.disabled=!0,a.textContent="?????? User-Imported / Custom ??????",a.style.fontWeight="bold",a.style.color="var(--text-dim)",t.modelInput.appendChild(a)}o.forEach(([a,l])=>t.modelInput.appendChild(r(a,l)));const s=document.createElement("option");s.value="custom",s.textContent="Custom Profile...",s.style.fontWeight="bold",s.style.color="var(--accent)",t.modelInput.appendChild(s),e&&(Object.prototype.hasOwnProperty.call(H,e)||e==="custom")?t.modelInput.value=e:t.modelInput.value||(t.modelInput.value="reterminal_e1001"),t.customHardwarePanel.updateVisibility()}function Ga(t){const e=t.modeSleep?.checked,n=t.modeDaily?.checked,i=t.modeDeepSleep?.checked,o=t.modeManual?.checked;t.sleepRow&&(t.sleepRow.style.display=e||i?"flex":"none"),t.dailyRefreshRow&&(t.dailyRefreshRow.style.display=n?"flex":"none"),t.deepSleepRow&&(t.deepSleepRow.style.display=i?"block":"none"),t.deepSleepOptionsRow&&(t.deepSleepOptionsRow.style.display=i?"flex":"none");const r=p.settings.lcdEcoStrategy||"backlight_off";t.dimTimeoutRow&&(t.dimTimeoutRow.style.display=r==="dim_after_timeout"?"flex":"none");const s=t.renderingModeInput?.value||p.settings.renderingMode||"direct",a=s==="lvgl"||s==="direct"||s==="c",l=s==="oepl"||s==="opendisplay";t.powerStrategySection&&(t.powerStrategySection.style.display=a?"block":"none"),t.protocolHardwareSection&&(t.protocolHardwareSection.style.display=l?"block":"none"),t.deviceModelField&&(t.deviceModelField.style.display=l?"none":"block");const c=!n&&!o;if(t.refreshIntervalRow&&(t.refreshIntervalRow.style.display=c?"block":"none"),t.autoCycleRow&&(t.autoCycleRow.style.display=t.autoCycleEnabled?.checked?"flex":"none"),t.deepSleepStayAwakeEntityRow){const d=document.getElementById("setting-deep-sleep-stay-awake");t.deepSleepStayAwakeEntityRow.style.display=d?.checked?"flex":"none"}t.customHardwarePanel.updateVisibility(),t.protocolHardwarePanel.updateStrategyDisplay()}function et(t){const e=document.createElement("div");return e.textContent=t||"",e.innerHTML}function js(t,e,n){if(t&&e&&e[t]){let o=e[t].name||t||"Unknown";return n.includes(t)||(o+=" (untested)"),o}const i={reterminal_e1001:"E1001 (Mono)",reterminal_e1002:"E1002 (Color)",trmnl:"TRMNL",esp32_s3_photopainter:"PhotoPainter (7-Color)"};return t&&(i[t]||t)||"Unknown"}function Ra(t,e,n,i){return t.map(o=>{const r=o.id===e,s=t.filter(a=>a.name===o.name).length>1;return`
                <tr style="border-bottom: 1px solid var(--border-subtle); ${r?"background: var(--accent-soft);":""}">
                    <td style="padding: 8px 4px;">
                        ...
                        ${s?'<br><span style="font-size: 9px; color: var(--muted);">'+et(o.id)+"</span>":""}
                    </td>
                    <td style="padding: 8px 4px; font-size: 11px; color: var(--muted);">${js(o.device_model||o.device_type,n,i)}</td>
                    ...
                    <td style="padding: 8px 4px; text-align: right;">
                        <div style="display: flex; gap: 4px; justify-content: flex-end;">
                            ...
                            ${!r&&t.length>1?`<button type="button" class="btn btn-sm btn-secondary" style="font-size: 10px; padding: 4px 8px; color: var(--danger);" data-action="delete" data-id="${o.id}" data-name="${et(o.name)}">Delete</button>`:""}
                        </div>
                    </td>
                </tr>
            `}).join("")}function qs(){return`
            <div class="modal" style="max-width: 600px;">
                <div class="modal-header">
        ...
        `}function Vs(t,e){return t?Object.entries(t).map(([n,i])=>{let o=i.name||n;return e.includes(n)||(o+=" (untested)"),`<option value="${n}">${o}</option>`}).join(""):'<option value="reterminal_e1001">reTerminal E1001</option>'}function Xs(t){return`
                <div class="modal" style="max-width: 400px;">
                    <div class="modal-header">
            ...
            `}function It(t){return document.getElementById(t)}function Xn(t){return document.getElementById(t)}function re(t){const e=document.getElementById(t);if(!e)throw new Error(`Missing element: ${t}`);return e}function Ba(){return Vs(H,vt||[])}function Wa(t){const e=document.getElementById("layoutManagerModal");if(e)return t.modal=e,e;const n=document.createElement("div");n.id="layoutManagerModal",n.className="modal-backdrop hidden",n.innerHTML=qs(),X(n),t.modal=n;const i=re("layoutManagerClose"),o=re("layoutManagerNew"),r=re("layoutManagerImport"),s=re("layoutManagerFileInput");i.addEventListener("click",()=>t.close()),o.addEventListener("click",()=>t.showNewLayoutDialog()),r.addEventListener("click",()=>{s.click()}),s.addEventListener("change",l=>t.handleFileImport(l)),n.addEventListener("click",l=>{l.target===n&&t.close()});const a=document.getElementById("layoutManagerTableBody");return a&&a.addEventListener("click",l=>{const c=l.target instanceof HTMLElement?l.target.closest("button"):null;if(!c)return;const{action:d,id:u,name:g}=c.dataset;d==="load"&&u&&t.loadLayout(u),d==="export"&&u&&t.exportLayout(u),d==="delete"&&u&&g&&t.deleteLayout(u,g)}),n}function Ks(t){const e=document.getElementById("newLayoutModal");if(e)return e;const n=document.createElement("div");return n.id="newLayoutModal",n.className="modal-backdrop hidden",n.innerHTML=Xs(t.generateDeviceOptions()),X(n),re("newLayoutClose").addEventListener("click",()=>{n.classList.add("hidden")}),re("newLayoutCancel").addEventListener("click",()=>{n.classList.add("hidden")}),re("newLayoutConfirm").addEventListener("click",()=>{t.handleCreateLayoutConfirm()}),re("newLayoutName").addEventListener("keydown",i=>{i.key==="Enter"?(i.preventDefault(),t.handleCreateLayoutConfirm()):i.key==="Escape"&&n.classList.add("hidden"),i.stopPropagation()}),n.addEventListener("click",i=>{if(i.target===n){const o=It("newLayoutName");document.activeElement!==o&&n.classList.add("hidden")}}),n}function Na(t){const e=Ks(t),n=It("newLayoutName");if(!n)return;const i=t.layouts.length;n.value=`Layout ${i+1}`;const o=H?Object.keys(H)[0]:"reterminal_e1001",r=Xn("newLayoutDeviceType");r&&(r.value=o),e.classList.remove("hidden"),setTimeout(()=>n.focus(),100)}function Fa(t){const e=It("newLayoutName"),n=Xn("newLayoutDeviceType"),i=e?.value.trim()||"",o=n?.value||"reterminal_e1001";if(!i){alert("Please enter a layout name.");return}const r=document.getElementById("newLayoutModal");r&&r.classList.add("hidden"),t.createLayout(i,o)}const Kn=t=>new Promise(e=>setTimeout(e,t));async function Js(t){const e=await $(`${t}/layouts`,{headers:z()});if(!e.ok)throw new Error(`Failed to load layouts: ${e.status}`);return e.json()}async function Zs(t,e){const n=await $(`${t}/layouts/${e}`,{headers:z()});if(!n.ok)throw new Error(`Failed to load layout: ${n.status}`);return n.json()}async function Qs(t,e){return $(`${t}/layouts/${e}`,{method:"POST",headers:{...z(),"Content-Type":"text/plain"},body:JSON.stringify({action:"delete"})})}async function ea(t,e){return $(`${t}/layouts`,{method:"POST",headers:{...z(),"Content-Type":"text/plain"},body:JSON.stringify(e)})}async function ta(t,e,n,i){const o=`${t}/import${n?"?overwrite=true":""}`;return $(o,{method:"POST",headers:i(),body:JSON.stringify(e)})}function na(t){return t instanceof Error?t.message:String(t)}function Le(){return F||""}async function za(t){if(!N()){t.setStatus("Not connected to Home Assistant","error");return}try{const e=await Js(Le());t.layouts=e.layouts||[],e.last_active_layout_id&&t.layouts.some(n=>n.id===e.last_active_layout_id)&&(!p?.currentLayoutId||p.currentLayoutId==="reterminal_e1001")&&t.layouts.find(i=>i.id===e.last_active_layout_id)&&e.last_active_layout_id!==p?.currentLayoutId&&(_.log(`[LayoutManager] Syncing to last active layout: ${e.last_active_layout_id}`),t.currentLayoutId=e.last_active_layout_id,p&&typeof p.setCurrentLayoutId=="function"&&p.setCurrentLayoutId(e.last_active_layout_id)),t.renderLayoutList()}catch(e){_.error("[LayoutManager] Error loading layouts:",e),t.setStatus("Failed to load layouts","error")}}async function $a(t,e){if(N())try{t.setStatus("Loading layout...","info");const n=await Zs(Le(),e);n.device_id||(n.device_id=e),t.currentLayoutId=e,p&&typeof p.setCurrentLayoutId=="function"&&(p.setCurrentLayoutId(e),_.log(`[LayoutManager] Set currentLayoutId to: ${e}`));const i=document.getElementById("canvas");if(i){const o=i.querySelector(".canvas-grid");i.innerHTML="",o&&i.appendChild(o),_.log("[LayoutManager] Cleared canvas before loading layout")}document.querySelectorAll(".graph-axis-label").forEach(o=>o.remove()),typeof ue=="function"&&ue(n),p&&p.currentLayoutId!==e&&(p.setCurrentLayoutId(e),_.log(`[LayoutManager] Re-set currentLayoutId to: ${e} (was changed by loadLayoutIntoState)`)),typeof L=="function"&&typeof C<"u"&&L(C.LAYOUT_IMPORTED,n),t.setStatus(`Loaded: ${n.name||e}`,"success"),t.renderLayoutList(),setTimeout(()=>t.close(),500)}catch(n){_.error("[LayoutManager] Error loading layout:",n),t.setStatus("Failed to load layout","error")}}async function Ya(t,e){if(!N())return;let n=null;try{const i=`${F}/export?id=${e}`,o=await $(i,{headers:z()});if(!o.ok){const l=await o.json().catch(()=>({}));throw new Error(l.error||`Export failed: ${o.status}`)}const r=await o.json(),s=new Blob([JSON.stringify(r,null,2)],{type:"application/json"});n=URL.createObjectURL(s);const a=document.createElement("a");a.href=n,a.download=`${e}_layout.json`,document.body.appendChild(a),a.click(),document.body.removeChild(a),t.setStatus("Export started...","success")}catch(i){_.error("[LayoutManager] Error exporting layout:",i),t.setStatus("Failed to export layout","error")}finally{n&&URL.revokeObjectURL(n)}}async function Ua(t,e,n){if(N()&&confirm(`Are you sure you want to delete "${n}"?

This cannot be undone.`)){t.setStatus("Deleting layout...","info");try{const i=await Qs(Le(),e);if(!i.ok){const o=await i.json().catch(()=>({}));if(o.error==="cannot_delete_last_layout"){t.setStatus("Cannot delete the last layout","error");return}throw new Error(o.error||`Delete failed: ${i.status}`)}t.setStatus(`Deleted: ${n}`,"success"),await t.loadLayouts()}catch(i){_.warn("[LayoutManager] Network error during delete, verifying if operation completed..."),await Kn(1500),await t.loadLayouts(),t.layouts.some(r=>r.id===e)?(_.error("[LayoutManager] Error deleting layout:",i),t.setStatus("Failed to delete layout","error")):(_.log("[LayoutManager] Layout was successfully deleted (verified after refresh)"),t.setStatus(`Deleted: ${n}`,"success"))}}}async function ja(t,e,n="reterminal_e1001"){if(!N())return;let i=e.toLowerCase().replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"");i||(i="layout");const o=`${i}_${Date.now()}`;t.setStatus("Creating layout...","info");let r=!1;try{const d=await ea(Le(),{id:o,name:e,device_type:n,device_model:n});if(!d.ok){const u=await d.json().catch(()=>({}));throw new Error(u.error||`Create failed: ${d.status}`)}r=!0}catch(d){if(_.warn("[LayoutManager] Network error during create, verifying if operation completed..."),await Kn(1500),await t.loadLayouts(),t.layouts.some(g=>g.id===o))_.log("[LayoutManager] Layout was successfully created (verified after refresh)"),r=!0;else{_.error("[LayoutManager] Error creating layout:",d),t.setStatus("Failed to create layout","error");return}}if(!r)return;t.setStatus(`Created: ${e}`,"success"),await t.loadLayouts();const s=H[n],a=s&&s.features&&s.features.epaper,l=s&&s.features&&s.features.lvgl,c=a&&!l?"direct":"lvgl";_.log(`[LayoutManager] New layout ${o} detected device type. isEpaper=${a}, hasLvgl=${l}. Setting initial renderingMode to: ${c}`),p&&(p.setPages([{id:"page_0",name:"Page 1",widgets:[]}]),p.setCurrentPageIndex(0),p.updateSettings({renderingMode:c,device_model:n}),_.log("[LayoutManager] Cleared state and set initial settings before loading new layout")),await t.loadLayout(o),p&&(p.setDeviceModel(n),typeof L=="function"&&typeof C<"u"&&L(C.STATE_CHANGED),_.log(`[LayoutManager] Created layout '${o}' with device_model: ${n}, pages: ${p.pages?.length}, widgets: ${p.getCurrentPage()?.widgets?.length||0}`))}async function qa(t,e){const n=e.target instanceof HTMLInputElement?e.target:null,i=n?.files?.[0];if(i){try{const o=await i.text(),r=JSON.parse(o);if(!r.pages&&!r.device_id){t.setStatus("Invalid layout file","error");return}await t.importLayout(r)}catch(o){_.error("[LayoutManager] Error importing file:",o),t.setStatus(`Failed to import file: ${na(o)}`,"error")}n&&(n.value="")}}async function Va(t,e,n=!1){if(N())try{const i=await ta(Le(),e,n,z),o=await i.json();if(!i.ok){if(o.error==="layout_exists"){confirm(`A layout with ID "${o.existing_id}" already exists.

Do you want to overwrite it?`)&&await t.importLayout(e,!0);return}throw new Error(o.error||`Import failed: ${i.status}`)}t.setStatus(`Imported: ${o.name||o.id}`,"success"),await t.loadLayouts()}catch(i){_.error("[LayoutManager] Error importing layout:",i),t.setStatus("Failed to import layout","error")}}export{ca as $,p as A,ha as B,ct as C,H as D,C as E,ga as F,$e as G,ma as H,V as I,Hn as J,ni as K,_ as L,Qn as M,$ as N,z as O,ya as P,xa as Q,_a as R,vt as S,ba as T,$n as U,va as V,Cs as W,Sa as X,wa as Y,Ea as Z,da as _,fn as a,pa as a0,Da as a1,ua as a2,K as a3,la as a4,fa as a5,Bs as a6,ka as a7,Ca as a8,La as a9,Ia as aa,P as ab,U as ac,Pr as ad,wt as ae,Yt as af,jn as ag,Ma as ah,ra as ai,sa as aj,Ta as ak,Aa as al,Pa as am,Ts as an,aa as ao,Ga as b,G as c,J as d,L as e,Wa as f,za as g,N as h,et as i,js as j,$a as k,Te as l,Ya as m,Ua as n,W as o,Ha as p,Ks as q,Ra as r,A as s,Na as t,Oa as u,Fa as v,Ba as w,ja as x,qa as y,Va as z};
