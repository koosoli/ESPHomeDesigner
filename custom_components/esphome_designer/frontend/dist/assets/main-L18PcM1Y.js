const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./plugin-CL6g4GaG.js","./text_utils-DPZlj6Oi.js","./plugin-CCJiZ1ha.js","./template_converter-Dp-G9w6d.js","./plugin-ClhWao6m.js","./plugin-C3o3T4QG.js"])))=>i.map(i=>d[i]);
...
`))};class od extends We{constructor(){super(),this.fonts=new Ul,this.yaml=new jl,this.reset()}reset(){this.fonts&&this.fonts.reset(),this.usedPlugins=new Set}async generate(e){if(!e)return console.error("ESPHomeAdapter: Missing layout"),"";this.reset();const n=e.pages||[],o=e.deviceModel||(g?g.deviceModel:null)||window.currentDeviceModel||"reterminal_e1001",i=B||{},s=window.DEVICE_PROFILES||{};let a={...i,...s}[o]||{};if(o==="custom"&&e.customHardware){const C=e.customHardware;a={id:"custom",name:"Custom Device",chip:C.chip||"esp32-s3",displayPlatform:C.displayDriver||"generic_st7789",displayModel:C.displayModel,resolution:{width:C.resWidth||800,height:C.resHeight||480},shape:C.shape||"rect",pins:{i2c:C.pins?.sda?{sda:C.pins.sda,scl:C.pins.scl}:null,spi:C.pins?.clk?{clk:C.pins.clk,mosi:C.pins.mosi}:null,display:{cs:C.pins?.cs,dc:C.pins?.dc,reset:C.pins?.rst,busy:C.pins?.busy}},features:{psram:!!C.psram,lcd:C.tech==="lcd",epaper:C.tech==="epaper",touch:C.touchTech&&C.touchTech!=="none"},backlight:C.pins?.backlight?{platform:"gpio",pin:C.pins.backlight}:null,touch:C.touchTech&&C.touchTech!=="none"?{platform:C.touchTech,sda:C.pins?.sda,scl:C.pins?.scl,interrupt_pin:C.pins?.touch_int,reset_pin:C.pins?.touch_rst}:null}}let l=!!(a.features&&(a.features.lvgl||a.features.lv_display));const c=e.renderingMode||(g?g.settings?.renderingMode:null);if(c==="direct"?(l=!1,v.log("[ESPHomeAdapter] Rendering mode set to 'direct', skipping LVGL generation")):c==="lvgl"&&(l=!0,v.log("[ESPHomeAdapter] Rendering mode set to 'lvgl', forcing LVGL generation")),!l)for(const C of n){if(C.widgets){for(const O of C.widgets.filter(W=>!W.hidden))if(O.type.startsWith("lvgl_")){l=!0;break}}if(l)break}const d=[];e.isSelectionSnippet||(d.push(...this.yaml.generateInstructionHeader(a,e)),d.push(...this.yaml.generateSystemSections(a,e)),d.push(""));const u=a.features?.lcd?"my_display":"epaper_display";this.preProcessWidgetsPromise=this.preProcessWidgets(n),await this.preProcessWidgetsPromise;let h=null;a.isPackageBased&&(a.isOfflineImport&&a.content?h=a.content:a.hardwarePackage&&(h=await this.fetchHardwarePackage(a.hardwarePackage)));const p=[];n.forEach((C,O)=>{C.widgets&&C.widgets.forEach(W=>{W.hidden||(W._pageIndex=O,p.push(W))})});const f=new Set,y=new Set,m=new Set,_=new Map,b={widgets:p,profile:a,layout:e,displayId:u,adapter:this,isLvgl:l,seenEntityIds:f,seenSensorIds:y,seenTextEntityIds:m,pendingTriggers:_,appState:g},w=[],x=[];U.onExportEsphome({...b,lines:x}),w.push("- id: display_page","  type: int","  restore_value: false","  initial_value: '0'");const E=!!(a.features&&(a.features.epaper||a.features.epd)),S=!!(a.features&&a.features.lcd)||!E,k=e.refreshInterval||(S?60:e.deepSleepInterval||600);w.push("- id: page_refresh_default_s","  type: int","  restore_value: true",`  initial_value: '${k}'`),w.push("- id: page_refresh_current_s","  type: int","  restore_value: false","  initial_value: '60'"),w.push("- id: last_page_switch_time","  type: uint32_t","  restore_value: false","  initial_value: '0'"),U.onExportGlobals({...b,lines:w}),x.length>0&&(e.plugin_includes=x),a.isPackageBased||(d.length=0,e.isSelectionSnippet||(d.push(...this.yaml.generateInstructionHeader(a,e)),d.push(...this.yaml.generateSystemSections(a,e)),d.push(""))),w.length>0&&!e.isSelectionSnippet&&(d.push("globals:"),d.push(...w.map(C=>"  "+C))),!(h&&h.includes("psram:"))&&a.features?.psram&&si&&d.push(...si(a)),!a.isPackageBased&&!e.isSelectionSnippet?(d.push("http_request:","  verify_ssl: false","  timeout: 20s","  buffer_size_rx: 4096"),ei&&d.push(...ei(a)),ti&&d.push(...ti(a)),Qn&&d.push(...Qn(a)),ri&&d.push(...ri(a)),ai&&d.push(...ai(a)),Zn&&d.push(...Zn(a)),li&&d.push(...li(a)),di&&d.push(...di(a)),d.some(O=>String(O).split(`
...
`).some(W=>W.trim()==="time:"))||(d.push("time:","  - platform: homeassistant","    id: ha_time"),y.add("ha_time")),a.features&&(a.pins?.batteryAdc&&(y.add("battery_voltage"),y.add("battery_level")),a.features.sht4x&&(y.add("sht4x_sensor"),y.add("sht4x_temperature"),y.add("sht4x_humidity")),(a.features.sht3x||a.features.sht3xd)&&(y.add("sht3x_sensor"),y.add("sht3x_temperature"),y.add("sht3x_humidity")),a.features.shtc3&&(y.add("shtc3_sensor"),y.add("shtc3_temperature"),y.add("shtc3_humidity"))),ii&&d.push(...ii(a,[],u,p));const M=[];U.onExportNumericSensors({...b,lines:M,mainLines:d});const R=this.processPendingTriggers(M,_,l,"on_value");R.length>0&&(d.some(C=>C==="sensor:")||d.push("sensor:"),d.push(...R.flatMap(C=>C.split(`
`).map(O=>"  "+O))));const G=Ql(n,b);if(G.length>0){d.some(O=>O==="sensor:")||d.push("sensor:");const C=this.processPendingTriggers(G,_,l,"on_value");d.push(...C.flatMap(O=>O.split(`
`).map(W=>"  "+W)))}const H=ed(n,b),N=[];U.onExportTextSensors({...b,lines:N}),H.length>0&&N.push(...H);const $=this.processPendingTriggers(N,_,l,"on_value");$.length>0&&(d.push("text_sensor:"),d.push(...$.flatMap(C=>C.split(`
`).map(O=>"  "+O))));const j=[];if(!a.isPackageBased&&st){const C=st(a,n.length,u,[]);C.length>0&&C[0].trim()==="binary_sensor:"?j.push(...C.slice(1).map(O=>O.startsWith("  ")?O.slice(2):O)):j.push(...C)}const ae=p.filter(C=>C.type==="touch_area"||C.type==="template_nav_bar");let te=[];if(ae.length>0&&st){const C=st({features:{}},n.length,u,ae);if(C.length>0){const O=C[0]?.trim()==="binary_sensor:"?1:0;C.length>O&&(a.isPackageBased?te=C.slice(O):(j.push("# Touch Area Binary Sensors"),j.push(...C.slice(O).map(W=>W.startsWith("  ")?W.slice(2):W))))}}this._pendingTouchSensors=te,U.onExportBinarySensors({...b,lines:j});const V=this.processPendingTriggers(j,_,l,"on_state");V.length>0&&!a.isPackageBased&&(d.push("binary_sensor:"),d.push(...V.flatMap(C=>C.split(`
`).map(O=>"  "+O))));const se=td(n,b);if(se.length>0){d.some(O=>O==="binary_sensor:")||d.push("binary_sensor:");const C=this.processPendingTriggers(se,_,l,"on_state");d.push(...C.flatMap(O=>O.split(`
`).map(W=>"  "+W)))}if(!a.isPackageBased&&oi){const C=oi(a,n.length,u);C.length>0&&d.push(...C)}const ie=U.getAll(),F=["image","online_image","graph","qr_code"];ie.sort((C,O)=>{const W=F.indexOf(C.id),ce=F.indexOf(O.id);return W!==-1&&ce!==-1?W-ce:W!==-1?-1:ce!==-1?1:C.id.localeCompare(O.id)}),ie.forEach(C=>C.onExportComponents&&C.onExportComponents({...b,lines:d}));const le=Zl(n,e,a,b,this);d.push(...this.fonts.getLines(e.glyphsets,e.extendedLatinGlyphs));const q=this.yaml.generateScriptSection(e,n,a);q.length>0&&d.push(...q);let $e=!1;if(l&&pi){const C=pi(n,o,a,e);C&&C.length>0&&(d.push(...C),$e=!0)}if(a.isPackageBased){if(h)return id(h,le,this._pendingTouchSensors,a,e,l,d)}else{const C=ni?ni(a,e,l):[];d.push(...C);for(let O=0;O<d.length;O++)if(d[O].trim()==="display:"){let W=O+1;for(;W<d.length&&(d[W].startsWith("  ")||d[W].trim()==="");)W++;$e||d.splice(W,0,"    lambda: |-",...le.map(ce=>ce.trim()?"      "+ce:""));break}}return d.map(C=>C.trimEnd()).join(`
