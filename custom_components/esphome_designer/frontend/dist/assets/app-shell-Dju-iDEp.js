...
`.trim();try{let d="";o==="gemini"?d=await this.callGemini(r,a,s,c):o==="openai"?d=await this.callOpenAI(r,a,s,c):o==="openrouter"&&(d=await this.callOpenRouter(r,a,s,c));let u=d.trim();if(u.includes("```")){const y=u.match(/```(?:json)?\s*([\s\S]*?)\s*```/);y&&y[1]&&(u=y[1].trim())}const h=u.indexOf("["),g=u.indexOf("{");let m=-1,f=-1;h!==-1&&(g===-1||h<g)?(m=h,f=u.lastIndexOf("]")):g!==-1&&(m=g,f=u.lastIndexOf("}")),m!==-1&&f!==-1&&f>m&&(u=u.substring(m,f+1));try{const y=JSON.parse(u),_=Xt.validateResponse(y);if(!_.valid&&(b.warn("[AI] Validation failed:",_.errors),_.sanitized.length===0))throw new Error("AI returned invalid widget data: "+_.errors.join(", "));return _.sanitized}catch(y){b.warn("Fast JSON parse failed, trying repair...",y);try{const _=this.repairJson(u),v=JSON.parse(_),x=Xt.validateResponse(v);return x.valid||b.warn("[AI] Validation failed after repair:",x.errors),x.sanitized}catch(_){throw b.error("JSON repair failed:",_),new Error("AI returned malformed JSON (possibly truncated). Try a shorter prompt or a more powerful model.")}}}catch(d){throw b.error("AI processing failed:",d),d}}async callGemini(e,n,i,o){const r=`https://generativelanguage.googleapis.com/v1beta/models/${n}:generateContent?key=${e}`,a={contents:[{role:"user",parts:[{text:i+`
