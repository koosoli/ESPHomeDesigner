import { getHaToken } from "../../js/utils/env.js";
...
function insertTopLevelSectionEntries(
  lines,
  sectionName,
  entryLines,
  commentLine = "",
...
export function exportLVGL(w, { common, convertColor, getLVGLFont }) {
  ...
  return {
    label: {
      ...common,
      text: textLambda,
      text_font: getLVGLFont(
        fontFamily,
        quoteFontSize,
        fontWeight,
        p.italic_quote !== false,
      ),
      text_color: color,
      text_align:
...
export function exportDoc(w, context) {
  const { lines, addFont, getColorConst, getConditionCheck, getAlignX } =
    context;
  const p = w.props || {};
  const quoteFontSize = parseInt(p.quote_font_size || 18, 10);
  ...
  if (bgColorProp && bgColorProp !== "transparent") {
    lines.push(
      `        it.filled_rectangle(${w.x}, ${w.y}, ${w.width}, ${w.height}, ${getColorConst(bgColorProp)});`,
    );
  }
  ...
  if (borderWidth > 0) {
    ...
    for (let index = 0; index < borderWidth; index++) {
      lines.push(
        `        it.rectangle(${w.x} + ${index}, ${w.y} + ${index}, ${w.width} - 2 * ${index}, ${w.height} - 2 * ${index}, ${borderColor});`,
      );
    }
  }
  ...
  for (let index = 0; index < itemCount; index++) {
    ...
    if (p.show_author !== false)
      lines.push(
        `            std::string q_author = id(${quoteAuthorGlobal});`,
      );
    ...
    if (p.word_wrap !== false) {
      ...
      lines.push(
        `                        if (draw) it.printf(${alignX}, y_local, f, ${color}, ${esphomeAlign}, "%s", curr_line.c_str());`,
      );
      ...
      lines.push(`              };`);
      lines.push(
        `              int height_used = print_q(id(${quoteFontId}), q_h, true);`,
      );
      lines.push(`              y_curr += height_used;`);
      if (p.show_author !== false) {
        lines.push(`              if (!q_author.empty()) {`);
        lines.push(
          `                it.printf(${alignX}, y_curr, id(${authorFontId}), ${color}, ${esphomeAlign}, "\u2014 %s", q_author.c_str());`,
        );
      ...
      const esphomeAlign = `TextAlign::${textAlign}`;
      lines.push(
        `              it.printf(${alignX}, y_curr, id(${quoteFontId}), ${color}, ${esphomeAlign}, "\\"%s\\"", q_text.c_str());`,
      );
    ...
    if (index < itemCount - 1) {
      const alignXForLine = textAlign.includes("CENTER")
        ? `${w.x + w.width / 2} - ${w.width * 0.2}`
        : textAlign.includes("RIGHT")
          ? `${w.x + w.width} - ${w.width * 0.4}`
          : `${w.x}`;
      lines.push(`              y_curr += 4;`);
      lines.push(
        `              it.line(${alignXForLine}, y_curr, ${alignXForLine} + ${w.width * 0.4}, y_curr, ${color});`,
      );
      lines.push(`              y_curr += 8;`);
    }
...
export function onExportComponents(context) {
  ...
  const httpBufSize = context.layout?.httpBufferSizeRx ?? 4096;
  const haToken = getHaToken();
  const hasHttpRequest = lines.some((line) =>
    line.trim().startsWith("http_request:"),
  );
  if (!hasHttpRequest) {
    lines.push(
      "",
  ...
  for (const w of targets) {
    ...
    const safeIdPrefix = makeSafeId(`quote_${w.id}`, "");
    const feedUrl =
      p.feed_url || "https://www.brainyquote.com/link/quotebr.rss";
    const itemCount = p.item_count || 1;
    ...
    intervalEntries.push(
      `                max_response_buffer_size: ${httpBufSize}`,
    );
    if (haToken) {
      intervalEntries.push(`                request_headers:`);
      intervalEntries.push(
    ...
    intervalEntries.push(
      `                        ESP_LOGD("quote", "Raw body: %s", body.c_str());`,
    );
    ...
    if (itemCount === 1) {
      intervalEntries.push(
        `                          JsonVariant q_var = doc["quote"];`,
      );
      intervalEntries.push(
        `                          if (q_var.is<JsonObject>()) {`,
      );
      intervalEntries.push(
        `                            JsonObject q = q_var.as<JsonObject>();`,
      );
      intervalEntries.push(
        `                            std::string q_str = q["quote"] | "";`,
      );
      intervalEntries.push(`                            if (!q_str.empty()) {`);
      intervalEntries.push(
        `                              id(${safeIdPrefix}_text_global) = q_str;`,
      );
      if (p.show_author !== false)
        intervalEntries.push(
          `                              id(${safeIdPrefix}_author_global) = q["author"] | "Unknown";`,
        );
      intervalEntries.push(
        `                              ESP_LOGI("quote", "Fetched quote: %s", q_str.c_str());`,
      );
      intervalEntries.push(`                            }`);
      intervalEntries.push(`                          }`);
    } else {
      ...
      intervalEntries.push(
        `                              std::string a_str = q["author"] | "Unknown";`,
      );
      intervalEntries.push(
        `                              if (!q_str.empty()) {`,
      );
      for (let index = 0; index < itemCount; index++) {
        ...
        intervalEntries.push(
          `                                  id(${safeIdPrefix}_text_global_${index}) = q_str;`,
        );
        if (p.show_author !== false)
          intervalEntries.push(
            `                                  id(${safeIdPrefix}_author_global_${index}) = a_str;`,
          );
        intervalEntries.push(`                                }`);
      }
      intervalEntries.push(
        `                                ESP_LOGI("quote", "Fetched quote %d: %s", i, q_str.c_str());`,
      );
      intervalEntries.push(`                              }`);
      intervalEntries.push(`                            }`);
    ...
    intervalEntries.push(
      `                        - lambda: 'ESP_LOGW("quote", "HTTP Request failed with code: %d", response->status_code);'`,
    );
  }
  insertTopLevelSectionEntries(
    lines,
    "interval",
    intervalEntries,
    "# Quote RSS Widget Update Loop",
  );
  lines.push("");
}
...
export function onExportTextSensors(context) {
  ...
  for (const w of targets) {
    ...
    for (let index = 0; index < itemCount; index++) {
      ...
      lines.push(
        `  lambda: 'return id(${safeIdPrefix}_text_global${suffix});'`,
      );
      ...
      if (p.show_author !== false) {
        ...
        lines.push(
          `  lambda: 'return id(${safeIdPrefix}_author_global${suffix});'`,
        );
        lines.push(`  update_interval: 60s`);
      }
    }
