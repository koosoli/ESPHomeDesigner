...
export const renderProperties = (panel, widget) => {
    ...
    panel.addLabeledInputWithPicker("Entity ID", "text", widget.entity_id || "", (/** @type {string} */ value) => {
        ...
        const previousEntity = AppState.entityStates?.[widget.entity_id || ""];
        const titleWasAutoGenerated = widget.title === previousEntity?.attributes?.friendly_name;
        if (value && (!widget.title || titleWasAutoGenerated)) panel.autoPopulateTitleFromEntity(widget.id, value);
    }, widget);
